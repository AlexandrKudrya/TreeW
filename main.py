...
def create_game():
    ...
    if form.validate_on_submit():
        ...
        db_sess.add(ngame)
        game_data = {'data': {}}
        with open(f'game_logs/{ngame.id}', 'w') as cat_file:
...
def newgame():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            res = make_response(render_template('game_logs.html', title='Создание комнаты', qust=qust, form=form))
            ...
            with open(f'game_logs/{ngame.id}', 'r') as cat_file:
                data = json.load(cat_file)
            data['data'][current_user.id] = data['data'].get(current_user.id, []) + [[qust.answer, form.answer.data]]
            return res
        except IndexError:
            return redirect('/')
    elif len(key) > 0:
...
def check_my_games():
    ...
    return render_template('check_my_game.html', arr=current_games)


@app.route('/check_my_game/<id>', methods=['GET', 'POST'])
def check_my_game():
    return 'позже'
